...
M.signature_window = function(_, result, ctx, config)
  ...
  local curr_cursor_line = vim.api.nvim_win_get_cursor(0)[1]

  if winner then
    if curr_cursor_line > 3 then
...
local check_trigger_char = function(line_to_cursor, triggers)
  ...
  for _, trigger_char in ipairs(triggers) do
    local curr_char = line_to_cursor:sub(#line_to_cursor, #line_to_cursor)
    local prev_char =
      line_to_cursor:sub(#line_to_cursor - 1, #line_to_cursor - 1)

    if curr_char == trigger_char then
      return true
    end
    if curr_char == " " and prev_char == trigger_char then
      return true
    end
  end
...
local open_signature = function()
  ...
  if triggered then
    local params = util.make_poisiton_names()
    vim.lsp.buf_request(
      0,
